/****************************************************************************
** Meta object code from reading C++ file 'todomodel.h'
**
** Created by: The Qt Meta Object Compiler version 68 (Qt 6.5.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../todomodel.h"
#include <QtCore/qmetatype.h>

#if __has_include(<QtCore/qtmochelpers.h>)
#include <QtCore/qtmochelpers.h>
#else
QT_BEGIN_MOC_NAMESPACE
#endif


#include <memory>

#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'todomodel.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 68
#error "This file was generated using the moc from 6.5.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

#ifndef Q_CONSTINIT
#define Q_CONSTINIT
#endif

QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
QT_WARNING_DISABLE_GCC("-Wuseless-cast")
namespace {

#ifdef QT_MOC_HAS_STRINGDATA
struct qt_meta_stringdata_CLASSToDoModelENDCLASS_t {};
static constexpr auto qt_meta_stringdata_CLASSToDoModelENDCLASS = QtMocHelpers::stringData(
    "ToDoModel",
    "addList",
    "",
    "name",
    "getListModel",
    "ListModel*",
    "id",
    "deleteList",
    "index",
    "getListSize"
);
#else  // !QT_MOC_HAS_STRING_DATA
struct qt_meta_stringdata_CLASSToDoModelENDCLASS_t {
    uint offsetsAndSizes[20];
    char stringdata0[10];
    char stringdata1[8];
    char stringdata2[1];
    char stringdata3[5];
    char stringdata4[13];
    char stringdata5[11];
    char stringdata6[3];
    char stringdata7[11];
    char stringdata8[6];
    char stringdata9[12];
};
#define QT_MOC_LITERAL(ofs, len) \
    uint(sizeof(qt_meta_stringdata_CLASSToDoModelENDCLASS_t::offsetsAndSizes) + ofs), len 
Q_CONSTINIT static const qt_meta_stringdata_CLASSToDoModelENDCLASS_t qt_meta_stringdata_CLASSToDoModelENDCLASS = {
    {
        QT_MOC_LITERAL(0, 9),  // "ToDoModel"
        QT_MOC_LITERAL(10, 7),  // "addList"
        QT_MOC_LITERAL(18, 0),  // ""
        QT_MOC_LITERAL(19, 4),  // "name"
        QT_MOC_LITERAL(24, 12),  // "getListModel"
        QT_MOC_LITERAL(37, 10),  // "ListModel*"
        QT_MOC_LITERAL(48, 2),  // "id"
        QT_MOC_LITERAL(51, 10),  // "deleteList"
        QT_MOC_LITERAL(62, 5),  // "index"
        QT_MOC_LITERAL(68, 11)   // "getListSize"
    },
    "ToDoModel",
    "addList",
    "",
    "name",
    "getListModel",
    "ListModel*",
    "id",
    "deleteList",
    "index",
    "getListSize"
};
#undef QT_MOC_LITERAL
#endif // !QT_MOC_HAS_STRING_DATA
} // unnamed namespace

Q_CONSTINIT static const uint qt_meta_data_CLASSToDoModelENDCLASS[] = {

 // content:
      11,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // methods: name, argc, parameters, tag, flags, initial metatype offsets
       1,    1,   38,    2, 0x02,    1 /* Public */,
       4,    1,   41,    2, 0x102,    3 /* Public | MethodIsConst  */,
       7,    1,   44,    2, 0x02,    5 /* Public */,
       9,    0,   47,    2, 0x102,    7 /* Public | MethodIsConst  */,

 // methods: parameters
    QMetaType::Void, QMetaType::QString,    3,
    0x80000000 | 5, QMetaType::Int,    6,
    QMetaType::Void, QMetaType::Int,    8,
    QMetaType::Int,

       0        // eod
};

Q_CONSTINIT const QMetaObject ToDoModel::staticMetaObject = { {
    QMetaObject::SuperData::link<QAbstractListModel::staticMetaObject>(),
    qt_meta_stringdata_CLASSToDoModelENDCLASS.offsetsAndSizes,
    qt_meta_data_CLASSToDoModelENDCLASS,
    qt_static_metacall,
    nullptr,
    qt_incomplete_metaTypeArray<qt_meta_stringdata_CLASSToDoModelENDCLASS_t,
        // Q_OBJECT / Q_GADGET
        QtPrivate::TypeAndForceComplete<ToDoModel, std::true_type>,
        // method 'addList'
        QtPrivate::TypeAndForceComplete<void, std::false_type>,
        QtPrivate::TypeAndForceComplete<const QString &, std::false_type>,
        // method 'getListModel'
        QtPrivate::TypeAndForceComplete<ListModel *, std::false_type>,
        QtPrivate::TypeAndForceComplete<int, std::false_type>,
        // method 'deleteList'
        QtPrivate::TypeAndForceComplete<void, std::false_type>,
        QtPrivate::TypeAndForceComplete<int, std::false_type>,
        // method 'getListSize'
        QtPrivate::TypeAndForceComplete<int, std::false_type>
    >,
    nullptr
} };

void ToDoModel::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<ToDoModel *>(_o);
        (void)_t;
        switch (_id) {
        case 0: _t->addList((*reinterpret_cast< std::add_pointer_t<QString>>(_a[1]))); break;
        case 1: { ListModel* _r = _t->getListModel((*reinterpret_cast< std::add_pointer_t<int>>(_a[1])));
            if (_a[0]) *reinterpret_cast< ListModel**>(_a[0]) = std::move(_r); }  break;
        case 2: _t->deleteList((*reinterpret_cast< std::add_pointer_t<int>>(_a[1]))); break;
        case 3: { int _r = _t->getListSize();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = std::move(_r); }  break;
        default: ;
        }
    }
}

const QMetaObject *ToDoModel::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ToDoModel::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_CLASSToDoModelENDCLASS.stringdata0))
        return static_cast<void*>(this);
    return QAbstractListModel::qt_metacast(_clname);
}

int ToDoModel::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QAbstractListModel::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<QMetaType *>(_a[0]) = QMetaType();
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
